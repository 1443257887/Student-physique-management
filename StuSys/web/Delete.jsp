...
<div class="contact-form">
    <form method="post" id="contact" action="Delete.jsp">
        <div class="row">
            <div class="col-md-6 col-sm-12">
                <fieldset>
                    <input type="text" name="id" id="name" placeholder="学号(精确删除时选填)" class="contact-field"/>
                    <input type="text" name="minweight" placeholder="体重下限(kg)(模糊删除时选填)"
                           class="contact-field"/>
                    <input type="text" name="minheight" placeholder="身高下限(cm)(模糊删除时选填)"
                           class="contact-field"/>
                </fieldset>
            </div>
            <div class="col-md-6 col-sm-12">
                <fieldset>
                    <input type="text" name="name" id="email" placeholder="姓名(精确删除时选填)" class="contact-field"/>
                    <input type="text" name="maxweight" placeholder="体重上限(kg)(模糊删除时选填)"
                           class="contact-field"/>
                    <input type="text" name="maxheight" placeholder="身高上限(cm)(模糊删除时选填)"
                           class="contact-field"/>
                </fieldset>
            </div>
            <div class="col-lg-12">
            ...
            <div class="col-lg-12">
                <fieldset>
                    <button type="submit" id="form-submit" class="main-button">删除</button>
                </fieldset>
            </div>
        </div>
    </form>
    ...
    <table>
        ...
        <%
            ...
            if (id == null && name == null && minweight == 0.0 && maxweight == 999.9 && minheight == 0.0 && maxheight == 999.9) {
                ...
                int flag = StuADQU.delete(conn,id,name);
                if (flag != 0){
                    out.println("<h3>以下内容已删除</h3>");
                }else {
                    out.println("<h3>出错了或该记录不存在</h3>");
                }
            } else if ((id == null && name == null) && (minweight != 0.0 || maxweight != 999.9 || minheight != 0.0 || maxheight != 999.9)) {
                Connection conn = StuJDBCUtil.getConn();
                ...
                int flag = StuADQU.deletePlus(conn,minweight,maxweight,minheight,maxheight);
                if (flag != 0){
                    out.println("<h3>以上内容已删除</h3>");
                }else {
                    out.println("<h3>出错了或该记录不存在</h3>");
                }
            }else {
                out.println("<h3>错误！精确删除与范围删除不可同时填写</h3>");
            }
